...
public abstract class SidebarManager {
    ...
    HashMap<BPlayer, Sidebar> sidebars = new HashMap<>();
    HashMap<BPlayer, Sideline> sidelines = new HashMap<>();

    @SuppressWarnings("unused")
    private TimeProvider timeProvider = null;

    public synchronized void update() {
        for (Map.Entry<BPlayer, Sidebar> e : sidebars.entrySet()) {
            BPlayer p = e.getKey();
    ...
    public void build(BPlayer p, SidebarEditor e) {
    }

    public void join(BPlayer p) {
        Sidebar sb = new Sidebar(p);
        sidebars.put(p, sb);
        sidelines.put(p, new Sideline(sb));
    }

    public void leave(BPlayer p) {
